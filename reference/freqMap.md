...
- ...:
  ...
  :   The limits of the colour scale, in the form `c(lower, upper)`. For
      example, `limits = c(0, 100)` will set the colour scale to be
      between `0` and `100`. Values greater than `100` will be coloured
      as if they were `100`, and those lower than `0` will be coloured
      as if they were `0`. `limits` can be wider than the range of the
      data, which can be useful for ensuring multiple plots share the
      same colour scale.
